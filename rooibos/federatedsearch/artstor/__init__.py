...
class ArtstorSearch(FederatedSearch):
    ...
    def photoSearch(self, searchString="", page=1, per_page=50):
        ...
        url = settings.ARTSTOR_GATEWAY+'?query="'+urllib.quote(searchString)+'"&operation=searchRetrieve&version=1.1&maximumRecords='+str(per_page)+'&startRecord='+str(((int(page)-1) * per_page) + 1)
