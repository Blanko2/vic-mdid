...
from rooibos.mobile import *


class HTMLPresentation(object):
    ...
    # put html here
    def url(self):
        return url(r'^html/(?P<id>[\d]+)/(?P<name>[-\w]+)/$', self.view, name='viewers-html')

    def url_for_obj(self, obj):
        return reverse('viewers-html', kwargs={'id': obj.id, 'name': obj.name})
